...
namespace MindTouch.Dream.Test {
    ...
    public class MockPlug : IMockPlug {
        ...
        public delegate void MockInvokeDelegate(Plug plug, string verb, XUri uri, DreamMessage request, Result<DreamMessage> response);

        //--- Class Fields ---
        private static Dictionary<string, List<MockPlug>> _mocks = new Dictionary<string, List<MockPlug>>();
        ...
        public static void Register(XUri uri, MockInvokeDelegate mock) {
            MockEndpoint.Instance.Register(uri, mock);
        }
        ...
        public static IMockPlug Setup(XUri baseUri) {
            ...
            lock(_mocks) {
                if(!_mocks.TryGetValue(key, out mocks)) {
                    ...
                    MockEndpoint.Instance.AllDeregistered += new EventHandler(Instance_AllDeregistered);
                    _mocks.Add(key, mocks);
                }
            }
            var mock = new MockPlug(baseUri);
            mocks.Add(mock);
            return mock;
        }
        ...
        /// <summary>
        /// Verify all <see cref="MockPlug"/> instances created with <see cref="Setup"/> since the last <see cref="DeregisterAll"/> call.
        /// </summary>
        /// <remarks>
        ...
        private int _times;
        private PlugCalled _verifiable;

        //--- Constructors ---
        private MockPlug(XUri uri) {
            _uri = uri;
        }
        ...
        private int GetMatchScore(string verb, XUri uri, DreamMessage request) {
            ...
            if(!string.IsNullOrEmpty(_uri.User) && !_uri.User.EqualsInvariant(uri.User)) {
                return 0;
            }
            if(!string.IsNullOrEmpty(_uri.Password) && !_uri.Password.EqualsInvariant(uri.Password)) {
                return 0;
            }
            if(!uri.WithoutTrailingSlash().Path.EqualsInvariantIgnoreCase(_uri.WithoutTrailingSlash().Path)) {
                return 0;
            }
            score++;
            if(_uri.Params != null) {
        ...
        public IMockPlug With(string key, Predicate<string> valueCallback) {
            _queryMatchers.Add(new Tuplet<string, Predicate<string>>(key, valueCallback));
            return this;
        }

        public IMockPlug WithCredentials(string user, string password) {
            _uri = _uri.WithCredentials(user, password);
            return this;
        }
        ...
        public IMockPlug WithHeader(string key, Predicate<string> valueCallback) {
        ...
        void IMockPlug.Verify(PlugCalled called) {
            ((IMockPlug)this).Verify(TimeSpan.FromSeconds(5), called);
        }

        IMockPlug IMockPlug.ExpectAtLeastOneCall() {
            _verifiable = PlugCalled.AtLeastOnce();
            return this;
        }

        IMockPlug IMockPlug.ExpectCalls(PlugCalled called) {
            _verifiable = called;
            return this;
        }
        ...
        void IMockPlug.Verify(TimeSpan timeout, PlugCalled called) {
            while(true) {
                var verified = called.Verify(_times, timeout);
                if(verified == PlugCalled.Result.Ok) {
                    return;
                }
                if(verified == PlugCalled.Result.TooMany) {
                    break;
                }
                ...
                _log.DebugFormat("waiting on {0} with {1:0.00}ms left in timeout", _uri, timeout.TotalMilliseconds);
                var stopwatch = Stopwatch.StartNew();
            ...
            throw new MockPlugException(string.Format("{0} was called {1} times before timeout.", _uri, _times));
        }
        #endregion
    }
    ...
    /// </summary>
    public class AutoMockPlug : IDisposable {
        ...
        internal AutoMockPlug(XUri baseUri) {
            _baseUri = baseUri;
            MockEndpoint.Instance.Register(_baseUri, Invoke);
        }
