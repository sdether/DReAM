...
namespace MindTouch {
    ...
    public static class SysUtil {
        ...
        static SysUtil() {
            ...
            if(!bool.TryParse(ConfigurationManager.AppSettings["async-io"], out UseAsyncIO)) {
                UseAsyncIO = !_isMono;
            }

            // intiailzie xml nametable
            int capacity;
            if(int.TryParse(ConfigurationManager.AppSettings["xmlnametable-capacity"], out capacity)) {
