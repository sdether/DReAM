...
namespace MindTouch.Traum.Test {
    ...
    /// </list>
    /// </remarks>
    public class MockPlug2 : IMockPlug2 {
        ...
        /// <summary>
        /// Delegate for registering a callback on Uri/Child Uri interception via <see cref="MockPlug2.Register(XUri,Test.MockPlug.MockInvokeDelegate)"/>.
        /// </summary>
        ...
        /// <summary>
        /// Setup a new <see cref="MockPlug2"/> interceptor candidate for a uri and its child paths.
        /// </summary>
        /// <remarks>
        ...
        public static IMockPlug2 Setup(string baseUri) {
        ...
        /// <returns>A new interceptor instance that may intercept the uri, depending on its additional matching parameters.</returns>
        public static IMockPlug2 Setup(string baseUri, string name) {
        ...
        /// <param name="baseUri">Base Uri to intercept.</param>
        /// <returns>A new interceptor instance that may intercept the uri, depending on its additional matching parameters.</returns>
        public static IMockPlug2 Setup(XUri baseUri) {
            _setupcounter++;
        ...
        /// <param name="name">Debug name for setup</param>
        /// <returns>A new interceptor instance that may intercept the uri, depending on its additional matching parameters.</returns>
        public static IMockPlug2 Setup(XUri baseUri, string name) {
        ...
        public static IMockPlug2 Setup(XUri baseUri, string name, int endPointScore) {
            ...
            lock(_mocks) {
                if(!_mocks.TryGetValue(key, out mocks)) {
                    ...
                    MockInvokeDelegate callback = (plug, verb, uri, request) => {
                        ...
                        if(bestMatch == null) {
                            _log.Debug("no match");
                            return TaskUtil.Result(DreamMessage2.Ok(new XDoc("empty")));
                        } else {
                            _log.DebugFormat("[{0}] matched", bestMatch.Name);
                            return TaskUtil.Result(bestMatch.Invoke(verb, uri, request));
                        }
                    };
            ...
            var mock = new MockPlug2(baseUri, name);
            mocks.Add(mock);
            return mock;
        }
        ...
        /// <summary>
        /// Verify all <see cref="MockPlug2"/> instances created with <see cref="Setup(XUri)"/> since the last <see cref="DeregisterAll"/> call.
        /// </summary>
        /// <remarks>
        ...
        public static void VerifyAll(TimeSpan timeout) {
            ...
            foreach(var mock in verifiable) {
                ...
                ((IMockPlug2)mock).Verify(timeout);
                stopwatch.Stop();
        ...
        //--- Constructors ---
        private MockPlug2(XUri uri, string name) {
            _uri = uri;
            Name = name;
        }
        ...
        private DreamMessage2 Invoke(string verb, XUri uri, DreamMessage2 request) {
            ...
            if(_response == null) {
                _response = DreamMessage2.Ok(new XDoc("empty"));
            }
        ...
        IMockPlug2 IMockPlug2.Verb(string verb) {
            _verb = verb;
            return this;
        }

        IMockPlug2 IMockPlug2.At(string[] path) {
            _uri = _uri.At(path);
            return this;
        }

        IMockPlug2 IMockPlug2.With(string key, string value) {
            _uri = _uri.With(key, value);
            return this;
        }

        IMockPlug2 IMockPlug2.With(string key, Predicate<string> valueCallback) {
            _queryMatchers.Add(new Tuple<string, Predicate<string>>(key, valueCallback));
            return this;
        }

        IMockPlug2 IMockPlug2.WithTrailingSlash() {
            _uri = _uri.WithTrailingSlash();
            _matchTrailingSlashes = true;
            return this;
        }

        IMockPlug2 IMockPlug2.WithoutTrailingSlash() {
            _uri = _uri.WithoutTrailingSlash();
            _matchTrailingSlashes = true;
            return this;
        }

        IMockPlug2 IMockPlug2.WithBody(XDoc request) {
            _request = request;
        ...
        IMockPlug2 IMockPlug2.WithHeader(string key, string value) {
            _headers[key] = value;
            return this;
        }

        IMockPlug2 IMockPlug2.WithHeader(string key, Predicate<string> valueCallback) {
        ...
        IMockPlug2 IMockPlug2.Returns(XDoc response) {
            var status = _response == null ? DreamStatus.Ok : _response.Status;
            var headers = _response == null ? null : _response.Headers;
            _response = new DreamMessage2(status, headers, response);
            return this;
        }

        IMockPlug2 IMockPlug2.WithResponseHeader(string key, string value) {
            _responseHeaders[key] = value;
            return this;
        }

        void IMockPlug2.Verify() {
            if(_verifiable == null) {
                return;
            }
            ((IMockPlug2)this).Verify(TimeSpan.FromSeconds(5), _verifiable);
        }

        void IMockPlug2.Verify(Times times) {
            ((IMockPlug2)this).Verify(TimeSpan.FromSeconds(5), times);
        }

        IMockPlug2 IMockPlug2.ExpectAtLeastOneCall() {
            _verifiable = Times.AtLeastOnce();
            return this;
        }

        IMockPlug2 IMockPlug2.ExpectCalls(Times called) {
            _verifiable = called;
            return this;
        }

        void IMockPlug2.Verify(TimeSpan timeout) {
            if(_verifiable == null) {
                return;
            }
            ((IMockPlug2)this).Verify(timeout, _verifiable);
        }

        void IMockPlug2.Verify(TimeSpan timeout, Times times) {
            while(true) {
    ...
    /// <summary>
    /// Provides and Arrange/Act/Assert mocking framework for intercepting and handling <see cref="Plug2"/> invocations.
    /// </summary>
    public class AutoMockPlug : MockPlug2.IMockInvokee, IDisposable {
        ...
        public delegate bool MockAutoInvokeDelegate(string verb, XUri uri, DreamMessage2 request, out DreamMessage2 response, out string failureReason);
        ...
        public void Expect(string verb, XUri uri, XDoc requestDoc, DreamMessage2 response) {
        ...
        int MockPlug2.IMockInvokee.EndPointScore { get { return int.MaxValue; } }
        XUri MockPlug2.IMockInvokee.Uri { get { return _baseUri; } }

        Task<DreamMessage2> MockPlug2.IMockInvokee.Invoke(Plug2 plug, string verb, XUri uri, DreamMessage2 request) {
            lock(this) {
                if(_failed) {
                    _log.DebugFormat("we've already failed, no point checking more expectations");
                    return TaskUtil.Result(DreamMessage2.InternalError());
                }
                ...
                if(_expectations.Count == _current) {
                    ...
                    _excess.Add(excess);
                    return TaskUtil.Result(excess.Call(verb, uri, requestDoc));
                }
                ...
                if(!expectation.Verify()) {
                    ...
                    _failed = true;
                    _resetEvent.Set();
                    return TaskUtil.Result(DreamMessage2.BadRequest("expectation failure"));
                }
                _current++;
                ...
                return TaskUtil.Result(expectation.GetResponse());
            }
        }
    ...
    public class ExcessInterception {
        ...
        internal DreamMessage2 Call(string verb, XUri uri, XDoc request) {
            _verb = verb;
            _uri = uri;
            _request = request;
            return DreamMessage2.BadRequest("unexpected call");
        }
    }
    ...
    public interface IMockInvokeExpectationParameter {
        ...
        /// <returns>Same instance of <see cref="IMockInvokeExpectationParameter"/>.</returns>
        IMockInvokeExpectationParameter Request(DreamMessage2 request);
        ...
        /// <returns>Same instance of <see cref="IMockInvokeExpectationParameter"/>.</returns>
        IMockInvokeExpectationParameter Response(DreamMessage2 response);
    ...
    internal class AutoMockInvokeExpectation : IMockInvokeExpectationParameter {
        ...
        private DreamMessage2 _response = DreamMessage2.Ok();
        private bool _called;
        private string _failure;
        private Func<XDoc, bool> _expectedRequestDocCallback;
        private DreamMessage2 _expectedRequest;
        ...
        public IMockInvokeExpectationParameter Request(DreamMessage2 request) {
        ...
        public IMockInvokeExpectationParameter Response(DreamMessage2 response) {
            _response = response;
            return this;
        }
        ...
        public DreamMessage2 GetResponse() { return _response; }

        public void Call(string verb, XUri uri, DreamMessage2 request) {
            _called = true;
            if(_autoInvokeDelegate != null) {
                DreamMessage2 response;
                string failure;
