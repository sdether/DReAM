...
namespace MindTouch.IO {
    ...
    public static class StreamUtil {
        ...
        //public static Task<int> Read(this Stream stream, byte[] buffer, int offset, int count, Result<int> result) {
        ...
        public static Task<long> CopyTo(this Stream source, Stream target, long length) {
            ...
            if((source == Stream.Null) || (length == 0)) {
                ...
                completion.SetResult(total);
            } else {

                // use new task environment so we don't copy the task state over and over again
                var copier = new AsyncCopier(source, target, length, completion);
                copier.Copy();
            }
