...
namespace MindTouch.Dream {
    ...
    public sealed class XUri : ISerializable {
        ...
        private static readonly NoCheck NO_CHECK = null;
        ...
        //--- Types ---
        private class NoCheck { }

        private class XUriToXDocValueConverter : IXDocTypeConverter {
            public string ToString(object value) {
                var uri = value as XUri ?? (value is Plug ? ((Plug)value).Uri : null);
                if(uri == null) {
                    throw new ArgumentException("Value to convert was not of Type XUri");
                }
                return uri.AsPublicUri().ToString();
            }
        }
        ...
        static XUri() {
            ...
            Localhost = new XUri("http", null, null, "localhost", 80, true, null, false, null, null, NO_CHECK);
            XDoc.AddConverter<XUri>(new XUriToXDocValueConverter());
            XDoc.AddConverter<Plug>(new XUriToXDocValueConverter());
        }
        ...
        public static bool TryParse(string text, out XUri uri) {
            ...
            uri = new XUri(scheme, user, password, host, port, null, segments, trailingSlash, @params, fragment, NO_CHECK);
            return true;
        }
        ...
        private XUri(string scheme, string user, string password, string host, int port, bool? defaultPort, string[] segments, bool trailingSlash, KeyValuePair<string, string>[] @params, string fragment, NoCheck unused) {
        ...
        public XUri At(params string[] segments) {
            ...
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, newSegments.ToArray(), trailingSlash, _params, Fragment, NO_CHECK);
        }
        ...
        public string GetRelativePathTo(XUri uri) {
            if((INVARIANT_IGNORE_CASE.Compare(this.Scheme, uri.Scheme) != 0) || (INVARIANT_IGNORE_CASE.Compare(this.Host, uri.Host) != 0) || (this.Port != uri.Port)) {
            ...
            for(; (i < Segments.Length) && (i < uri.Segments.Length); ++i) {
                if(INVARIANT_IGNORE_CASE.Compare(Segments[i], uri.Segments[i]) != 0) {
                    break;
                }
            }

            // use '..' path for unmatched segments
            for(int j = i; j < uri.Segments.Length; ++j) {
                if(result.Length > 0) {
                    result.Append("/");
                }
        ...
        public bool HasPrefix(XUri prefix) {
            ...
            if((INVARIANT_IGNORE_CASE.Compare(Scheme, prefix.Scheme) != 0) || (INVARIANT_IGNORE_CASE.Compare(Host, prefix.Host) != 0) || (Port != prefix.Port)) {
                return false;
            }
            ...
            for(; (commonSegments < Segments.Length) && (commonSegments < prefix.Segments.Length); ++commonSegments) {
                if(INVARIANT_IGNORE_CASE.Compare(Segments[commonSegments], prefix.Segments[commonSegments]) != 0) {
                    return false;
                }
            }
        ...
        public XUri ChangePrefix(XUri from, XUri to) {
            ...
            // check that prefix matches
            if((INVARIANT_IGNORE_CASE.Compare(Scheme, from.Scheme) != 0) || (INVARIANT_IGNORE_CASE.Compare(Host, from.Host) != 0) || (Port != from.Port)) {
            ...
            for(; (commonSegments < Segments.Length) && (commonSegments < from.Segments.Length); ++commonSegments) {
                if(INVARIANT_IGNORE_CASE.Compare(Segments[commonSegments], from.Segments[commonSegments]) != 0) {
                    break;
                }
            }
            ...
            // build new uri
            return new XUri(to.Scheme, User, Password, to.Host, to.Port, to.UsesDefaultPort, segments, TrailingSlash, Params, Fragment, NO_CHECK);
        }
        ...
        public string[] GetParams(string key) {
            if(_params != null) {
                ...
                for(int i = 0; i < _params.Length; ++i) {
                    if(INVARIANT_IGNORE_CASE.Compare(_params[i].Key, key) == 0) {
        ...
        public XUri WithParams(KeyValuePair<string, string>[] args) {
            ...
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, newParams, Fragment, NO_CHECK);
        }
        ...
        public XUri WithoutQuery() {
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, null, Fragment, NO_CHECK);
        }
        ...
        public XUri WithCredentials(string user, string password) {
            return new XUri(Scheme, user, password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, Params, Fragment, NO_CHECK);
        }
        ...
        public XUri WithCredentialsFrom(XUri uri) {
            return new XUri(Scheme, uri.User, uri.Password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, Params, Fragment, NO_CHECK);
        }
        ...
        public XUri WithoutCredentials() {
            return new XUri(Scheme, null, null, Host, Port, UsesDefaultPort, Segments, TrailingSlash, Params, Fragment, NO_CHECK);
        }
        ...
        public XUri WithFragment(string fragment) {
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, _params, fragment, NO_CHECK);
        }
        ...
        public XUri WithoutFragment() {
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, _params, null, NO_CHECK);
        }
        ...
        public XUri WithoutPathQueryFragment() {
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, null, true, null, null, NO_CHECK);
        }
        ...
        public XUri WithoutCredentialsPathQueryFragment() {
            return new XUri(Scheme, null, null, Host, Port, UsesDefaultPort, null, true, null, null, NO_CHECK);
        }
        ...
        public XUri WithoutParams(string key) {
            ...
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, TrailingSlash, rest.ToArray(), Fragment, NO_CHECK);
        }
        ...
        public XUri WithTrailingSlash() {
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, true, Params, Fragment, NO_CHECK);
        }
        ...
        public XUri WithoutTrailingSlash() {
            return new XUri(Scheme, User, Password, Host, Port, UsesDefaultPort, Segments, false, Params, Fragment, NO_CHECK);
        }
        ...
        public XUri WithPort(int port) {
            ...
            return new XUri(Scheme, User, Password, Host, port, null, Segments, TrailingSlash, Params, Fragment, NO_CHECK);
        }
        ...
        public override bool Equals(object obj) {
            ...
            // check scheme
            if(INVARIANT_IGNORE_CASE.Compare(Scheme, other.Scheme) != 0) {
                return false;
            }

            // check host
            if(INVARIANT_IGNORE_CASE.Compare(Host, other.Host) != 0) {
                return false;
            }
            ...
            // check path
            for(int i = 0; i < Segments.Length; ++i) {
                if(INVARIANT_IGNORE_CASE.Compare(Segments[i], other.Segments[i]) != 0) {
                    return false;
                }
            }
        ...
        public bool PathStartsWith(string[] segments) {
            ...
            for(int i = 0; i < segments.Length; ++i) {
                if(INVARIANT_IGNORE_CASE.Compare(Segments[i], segments[i]) != 0) {
                    return false;
                }
            }
            return true;
