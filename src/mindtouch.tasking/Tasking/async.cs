...
namespace MindTouch.Tasking {
    ...
    public static class Async {
        ...
        //--- Constructors ---
        static Async() {
            if(!int.TryParse(ConfigurationManager.AppSettings["threadpool-min"], out _minThreads)) {
                _minThreads = 4;
            }
            if(!int.TryParse(ConfigurationManager.AppSettings["threadpool-max"], out _maxThreads)) {
                _maxThreads = 200;
            }

            // check which global dispatch queue implementation to use
            int dummy;
            switch(ConfigurationManager.AppSettings["threadpool"]) {
            default:
            case "elastic":
