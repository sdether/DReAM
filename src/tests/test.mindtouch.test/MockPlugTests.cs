...
namespace MindTouch.Dream.Test {
    ...
    public class MockPlugTests {
        ...
        public void MockPlug_can_verify_call_via_VerifyAll() {
            MockPlug.Setup(new XUri("http://mock/foo")).ExpectCalls(PlugCalled.AtLeastOnce());
        ...
        public void Can_verify_call() {
            var mock = MockPlug.Setup(new XUri("http://mock/foo")).ExpectCalls(PlugCalled.AtLeastOnce());
            Assert.IsTrue(Plug.New("http://mock/foo").Get(new Result<DreamMessage>()).Wait().IsSuccessful);
            mock.Verify();
        }
        ...
        public void Can_verify_lack_of_call() {
            var mock = MockPlug.Setup(new XUri("http://mock/foo")).ExpectCalls(PlugCalled.Never());
        ...
        public void Can_pick_appropriate_MockPlug_based_on_verb() {
            ...
            Assert.IsTrue(Plug.New("http://mock/foo").Get(new Result<DreamMessage>()).Wait().IsSuccessful);
            a.Verify(2.Seconds(), PlugCalled.Never());
            c.Verify(0.Seconds(), PlugCalled.Never());
            b.Verify(PlugCalled.Once());
        }
        ...
        [Test]
        public void Can_check_for_credentials_in_uri() {
            var a = MockPlug.Setup(new XUri("http://mock/foo")).WithCredentials("b", "c");
            Assert.IsTrue(Plug.New(new XUri("http://mock/foo").WithCredentials("b", "c")).Get(new Result<DreamMessage>()).Wait().IsSuccessful);
            a.Verify(0.Seconds(), PlugCalled.Once());
        }

        [Test]
        public void Call_without_credentials_when_expects_does_not_trigger_setup() {
            var a = MockPlug.Setup(new XUri("http://mock/foo")).WithCredentials("b", "c");
            Assert.IsTrue(Plug.New(new XUri("http://mock/foo")).Get(new Result<DreamMessage>()).Wait().IsSuccessful);
            a.Verify(0.Seconds(), PlugCalled.Never());
        }
        ...
        public void Can_differentiate_multiple_plugs_and_their_call_counts() {
            ...
            var eek = MockPlug.Setup(new XUri("http://mock/foo")).At("eek").With("a", "b").ExpectCalls(PlugCalled.Exactly(3));
        ...
        public void Can_pick_appropriate_MockPlug_based_on_headers() {
            ...
            bar.Verify(2.Seconds(), PlugCalled.Never());
            baz.Verify(0.Seconds(), PlugCalled.Never());
            eek.Verify(PlugCalled.Once());
        }
