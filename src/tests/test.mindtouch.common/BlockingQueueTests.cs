...
namespace MindTouch.Dream.Test {
    ...
    public class BlockingQueueTests {
        ...
        public void One_producer_one_consumer_loop_manually() {
            ...
            BlockingQueue<string> q = new BlockingQueue<string>();
            Thread consumer = new Thread(SingleConsumerManualLoop);
            consumer.IsBackground = true;
            AutoResetEvent reset = new AutoResetEvent(false);
            consumer.Start(new Tuplet<int, IBlockingQueue<string>, List<string>, AutoResetEvent>(n, q, dequeued, reset));
            for(int i = 0; i < n; i++) {
                string guid = Guid.NewGuid().ToString();
                q.Enqueue(guid);
                enqueued.Add(guid);
        ...
        private void SingleConsumerManualLoop(object obj) {
            Tuplet<int, IBlockingQueue<string>, List<string>, AutoResetEvent> state = (Tuplet<int, IBlockingQueue<string>, List<string>, AutoResetEvent>)obj;
        ...
        public void One_producer_one_consumer_loop_with_foreach() {
            ...
            List<string> dequeued = new List<string>();
            BlockingQueue<string> q = new BlockingQueue<string>();
            Thread consumer = new Thread(SingleConsumerForeachLoop);
            consumer.IsBackground = true;
            AutoResetEvent reset = new AutoResetEvent(false);
            consumer.Start(new Tuplet<int, IBlockingQueue<string>, List<string>, AutoResetEvent>(n, q, dequeued, reset));
            for(int i = 0; i < n; i++) {
        ...
        private void MultiProducer(object obj) {
            Tuplet<BlockingQueue<string>, List<string>, int, ManualResetEvent> state =
                (Tuplet<BlockingQueue<string>, List<string>, int, ManualResetEvent>)obj;
            List<string> enqueued = new List<string>();
