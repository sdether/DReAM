...
namespace MindTouch.Dream.Test.Statsd {
    ...
    [TestFixture]
    public class CountingStatisticTests {

        [Test]
        public void Can_Convert_to_bytes_without_sampling() {
            var stat = new CountingStatistic("test.foo", 5);
            var bytes = stat.ToBytes(1);
            Assert.AreEqual("test.foo:5|c", bytes.FromBytes());
        }
        ...
        public void Can_Convert_to_bytes_with_10th_sampling() {
            ...
            Assert.AreEqual("test.foo:5|c|@0.1", bytes.FromBytes());
        }
        ...
        public void Can_Convert_to_bytes_with_100th_sampling() {
            ...
            Assert.AreEqual("test.foo:5|c|@0.01", bytes.FromBytes());
        }
    }
}
